...
def generate_paper_style_graph(
    df: pd.DataFrame,
    output_path: Path,
    true_leaves: set,
    mode: str = "endpoint",
    ...
    """
    Generates a top-to-bottom tree matching the paper's Figure 3 layout,
    with level indicators and nodes aligned by depth.
    """
    print(f"\n[INFO] Generating paper-style graph ({mode} mode)...")
    ...
    # 1. Build Paths, Counts, and Edges (same logic as generate_graph)
    node_counts = Counter()
    ...
    # 2. Collapse 1:1 parent-leaf relationships
    collapsed_nodes = {}
    collapsed_edges = set(edges)
    ...
    # Root nodes = nodes with no parents in the edge set
    root_nodes = [n for n in all_encountered_nodes if n not in parents_map]
    node_depth = {}
    ...
    while queue:
        ...
        for child in children_map.get(current, []):
            child_depth = node_depth[current] + 1
            if child not in node_depth or child_depth < node_depth[child]:
                node_depth[child] = child_depth
                queue.append(child)
    ...
    # 5. Add level label nodes and enforce ranking
    # Create invisible level label column on the left
    ...
    # Chain level labels with invisible edges to enforce vertical order
    ...
    # 6. Create rank=same subgraphs for each depth level
...
def main():
    ...
    print("=" * 60)
    generate_paper_style_graph(
        df, output_path, true_leaves, mode="endpoint", suffix="_paper_style"
    )
